...
namespace NUniTest.Service
{
    ...
    public class ChatServiceTests
    {
        ...
        // =============================
        // 6. SendMessageAsync - Successful message from team member
        // =============================
        [Test]
        public async Task SendMessageAsync_FromTeamMember_ShouldSendSuccessfully()
        {
            var dto = new SendMessageDto
            {
                ChatGroupId = 1,
            ...
            { 
                ...
                MentorId = 2, 
                TeamId = 1,
                Team = new Team { TeamId = 1, TeamName = "Test Team" },
                Mentor = new User { UserId = 2, FullName = "Mentor Name" }
            };
            ...
            { 
                MessageId = 1, 
                Content = "Hello mentor!",
                ReadStatuses = new List<MessageReadStatusDto>()
            };
            ...
            _chatGroupRepo.Setup(r => r.GetByIdIncludingAsync(
                ...
                .ReturnsAsync(chatGroup);
            _chatGroupRepo.Setup(r => r.GetByIdIncludingAsync(
        It.IsAny<Expression<Func<ChatGroup, bool>>>(),
        It.IsAny<Expression<Func<ChatGroup, object>>>(),
        It.IsAny<Expression<Func<ChatGroup, object>>>()
    ))
    .ReturnsAsync(chatGroup);
            ...
            _chatGroupRepo.Setup(r => r.Update(It.IsAny<ChatGroup>()));
            _chatMessageRepo.Setup(r => r.AddAsync(It.IsAny<ChatMessage>())).Returns(Task.CompletedTask);
            _uowMock.Setup(u => u.SaveAsync(It.IsAny<int?>())).ReturnsAsync(1);
            _chatMessageReadRepo.Setup(r => r.AddAsync(It.IsAny<ChatMessageRead>())).Returns(Task.CompletedTask);
            _chatMessageRepo.Setup(r => r.GetByIdIncludingAsync(
        It.IsAny<Expression<Func<ChatMessage, bool>>>(),
        It.IsAny<Expression<Func<ChatMessage, object>>[]>()
    ))
    .ReturnsAsync((Expression<Func<ChatMessage, bool>> predicate,
                   Expression<Func<ChatMessage, object>>[] includes) =>
    {
        var compiled = predicate.Compile();
        return compiled(message) ? message : null;
    });

            _mapperMock.Setup(m => m.Map<ChatMessageDto>(message)).Returns(messageDto);

            var result = await _service.SendMessageAsync(dto, 1);

            result.Should().NotBeNull();
            result.Content.Should().Be("Hello mentor!");
            result.ReadStatuses.Should().NotBeNull();

            _chatMessageRepo.Verify(r => r.AddAsync(It.IsAny<ChatMessage>()), Times.Once);
            _chatMessageReadRepo.Verify(r => r.AddAsync(It.IsAny<ChatMessageRead>()), Times.Once);
        }
        ...
        // =============================
        // 12. MarkAsReadAsync - Mark messages as read successfully
        // =============================
        [Test]
        public async Task MarkAsReadAsync_ShouldMarkUnreadMessagesAsRead()
        {
            var chatGroup = new ChatGroup { ChatGroupId = 1 };
    ...
    .ReturnsAsync(unreadMessages);
            ...
            _chatMessageReadRepo.Verify(r => r.AddRangeAsync(It.Is<IEnumerable<ChatMessageRead>>(
                reads => reads.Count() == 2)), Times.Once);
        }
